...
dn.set_gpu(0)
net1 = dn.load_net(configPath.encode("ascii"), weightsPath.encode("ascii"), 0)
meta1 = dn.load_meta(metaPath.encode("ascii"))

dn.set_gpu(1)
net2 = dn.load_net(configPath.encode("ascii"), weightsPath.encode("ascii"), 0)
meta2 = dn.load_meta(metaPath.encode("ascii"))

dn.set_gpu(2)
net3 = dn.load_net(configPath.encode("ascii"), weightsPath.encode("ascii"), 0)
meta3 = dn.load_meta(metaPath.encode("ascii"))

dn.set_gpu(3)
net4 = dn.load_net(configPath.encode("ascii"), weightsPath.encode("ascii"), 0)
meta4 = dn.load_meta(metaPath.encode("ascii"))
...
#   r = dn.detect(net, meta, im)
    nums = [(0, im), (1, im), (2, im), (3, im)]
    with ThreadPoolExecutor() as executor:
        r = [val for val in executor.map(f, nums)]
    print(r)
    return Response(status=200)
...
def f(x):
    if x[0] == 0: # gpu 0
        return dn.detect(net1, meta1, x[1])
    if x[0] == 1: # gpu 0
        return dn.detect(net2, meta2, x[1])
    if x[0] == 2: # gpu 3
        return dn.detect(net3, meta3, x[1])
    else: # gpu 4
        return dn.detect(net4, meta4, x[1])
