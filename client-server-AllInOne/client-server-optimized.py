...
#    response = requests.post(uri_init, json = {})
    ...
    while True:
        ...
        #print(frame.shape)
#        frame1 = {"Frame":frame.tolist()}
 #       frame2 = np.array(frame1["Frame"], dtype = "uint8")
  #      frame3 = np.reshape(frame2, (360, 640, 3))
   #     im = array_to_image(frame3)
#        frame = np.array(frame.tolist(), dtype = "uint8")
        time2 = time.time();
  #      print("Time to transform current frame to np array ", time2-time1)
   ...
   #     frame = np.reshape(frame, (360, 640, 3))
        ...
        print("Time to reshape array ", time3-time2)
        im = array_to_image(frame)
        time4 = time.time();
        print("Time to turn array into image ", time4-time3)
        dn.rgbgr_image(im)
        time5 = time.time();
        print("Time to turn image into gray image ", time5-time4)
